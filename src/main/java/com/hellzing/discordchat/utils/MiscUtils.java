...
public class MiscUtils
{
    ...
    {
        return DCConfig.discordToMCFormat.replaceAll("\\$1", message.getJDA().getTextChannelById(message.getChannelId()).getName()).replaceAll("\\$2", message.getAuthor().getUsername())
    ...
    {
        return DCConfig.deathMessageFormat.replaceAll("\\$1", getName(player)).replaceAll("\\$2", player.func_110142_aN().func_151521_b().getUnformattedText());
    }
    ...
    public static String createAchievementMessage(EntityPlayer player, Achievement achievement)
    {
        ...
        return DCConfig.achievementMessageFormat.replaceAll("\\$1", getName(player)).replaceAll("\\$2", achievementText.getUnformattedText());

    }

    public static String createLoggedInMessage(EntityPlayer player)
    {
        return DCConfig.playerJoinMessageFormat.replaceAll("\\$1", getName(player));
    }

    public static String createLoggedOutMessage(EntityPlayer player)
    {
        return DCConfig.playerLeaveMessageFormat.replaceAll("\\$1", getName(player));
    }
